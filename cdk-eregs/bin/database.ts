...
async function main() {
    ...
    // Get VPC configuration from SSM parameters
    const vpcId = await getParameterValue('/account_vars/vpc/id');
    const subnetIds = [
        await getParameterValue('/account_vars/vpc/subnets/private/a/id'),
        await getParameterValue('/account_vars/vpc/subnets/private/b/id')
    ];

    // Create Database Stack
    new DatabaseStack(app, `${stageConfig.getResourceName('database')}`, {
        environmentConfig: {
            vpcId,
            subnetIds,
        },
        ...
        description: `Database stack for ${stageConfig.environment}`
    }, stageConfig);
