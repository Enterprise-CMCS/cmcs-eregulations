...
export const SSM_PARAMETERS = {
  ...
  SYNTHESIZER: {
    /** CDK Synthesizer roles configuration */
    CONFIG: {
      path: '/cms/cloud/cdkSynthesizerConfig',
      required: true,
      description: 'CDK Synthesizer roles configuration',
      transform: (value: string) => JSON.parse(value),
      validate: (value: any) => {
        return value.qualifier &&
          value.cloudFormationExecutionRole &&
          value.deployRoleArn &&
          value.fileAssetPublishingRoleArn &&
          value.imageAssetPublishingRoleArn &&
          value.lookupRoleArn;
      },
    },
  } as SSMParameterGroup,
