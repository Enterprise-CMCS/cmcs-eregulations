...
class PartSitemap(Sitemap):
    ...
    def processChildren(self, children, title, part, last_updated):
        ...
        for child in children:
            if child.get('type', '') == 'subpart':
                results.append({'title': title, 'part': part, 'subpart': child['identifier'][0], 'last_updated':last_updated})
    ...
    def items(self):
        ...
        results = []
        for p in query:
            results.append({'title':p.title, 'part': p.name, 'last_updated': p.last_updated})
